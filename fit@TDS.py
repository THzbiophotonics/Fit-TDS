...
warnings.filterwarnings(
    "ignore"
)  # this is just to remove the 'devided by zero' runtime worning for low frequency
...
def Drude(drudeinput):
    ...
    if isdrude == 1:
        ...
        eps = eps + wp ** 2 / (
            1E0 + j * gamma * myglobalparameters.w - myglobalparameters.w ** 2
        )
        interm = interm + 2

    for i in range(0, n):
        chi = drudeinput[i * 3 + 1 + interm]
        w0 = drudeinput[i * 3 + 2 + interm]
        gamma = drudeinput[i * 3 + 3 + interm]
        eps = eps + chi * w0 ** 2 / (
            w0 ** 2 + j * gamma * myglobalparameters.w - myglobalparameters.w ** 2
        )
    return eps
...
def monerreur(drudeinput):
    ...
    erreur = (
        np.linalg.norm(
            (myinputdata.mytransferfunction - transferfunction(drudeinput))
            * myinputdatafromfile.Spulseinit
        )
        / myinputdata.mynorm
    )
    return erreur
...
myglobalparameters.t = (
    myinputdatafromfile.PulseInittotal[:, 0] * 1e-12
)  # this assumes input files are in ps
...
deltaz = (
    sanitised_input("\nEnter a value for the thickness uncertainty in %:", float, 0)
    / 100
)

myinputdata = mydata(mesdata[:, 1], myinputdatafromfile.Spulseinit, z)
monepsilon = dielcal(
    np.fft.rfft((mesdata[:, 1])) / myinputdatafromfile.Spulseinit, z, myglobalparameters
)
...
# =============================================================================
deltaT = (
    myglobalparameters.t[np.argmax(myinputdata.pulse)]
    - myglobalparameters.t[np.argmax(myinputdatafromfile.Pulseinit)]
...
deltaTTT = (
    myglobalparameters.t[np.argmin(myinputdata.pulse)]
    - myglobalparameters.t[np.argmin(myinputdatafromfile.Pulseinit)]
)  # retard entre les deux max
...
deltaTT = np.sum(np.square(myinputdata.pulse) * myglobalparameters.t) / np.sum(
    np.square(myinputdata.pulse)
) - np.sum(np.square(myinputdatafromfile.Pulseinit) * myglobalparameters.t) / np.sum(
    np.square(myinputdatafromfile.Pulseinit)
)  # retard entre les deux barycentre, attention pour que ca foncionne il faut que le rapport signal bruit soit le meme dans les deux cas !!
print(
    "############################################################################################"
)
print(
...
if mymodelstruct == 2:
    for i in range(0, 1):
        myvariables = [
            "Omega resonator/metasurface_" + str(i),
            "Tau 0 resonator/metasurface_" + str(i),
            "Tau 1 resonator/metasurface_" + str(i),
            "Tau 2 resonator/metasurface_" + str(i),
            "delta Theta resonator/metasurface_" + str(i),
        ]
else:
    myvariables = []
...
for i in range(0, n):
    myvariables = myvariables + ["Chi_" + str(i), "Omega0_" + str(i), "Gamma_" + str(i)]
...
for i in myvariables:
    while True:
        ...
        lb[compteur] = sanitised_input(
            "Enter a value for the minimum value of " + str(i) + ": \t", float, 0
        )
        up[compteur] = sanitised_input(
            "Enter a value for the maximum value of " + str(i) + ": \t", float, 0
        )
        if up[compteur] <= lb[compteur]:
            print(
                "The maximum of " + str(i) + " must be strictly higher than its minimum"
            )
            continue
...
if algo > 1:
    interm2 = 0
    opt_prob = Optimization("Dielctric modeling based on TDS pulse fitting", objfunc)
    if zvariable == 1:
    ...
    for i in range(0, n):
        opt_prob.addVar(
            "chi" + str(i),
            "c",
            lower=lb[1 + interm2 + 3 * i],
            upper=up[1 + interm2 + 3 * i],
            value=drudeinput[1 + interm2 + 3 * i],
        )  # pour drude
        opt_prob.addVar(
            "w" + str(i),
            "c",
            lower=lb[2 + interm2 + 3 * i],
            upper=up[2 + interm2 + 3 * i],
            value=drudeinput[2 + interm2 + 3 * i],
        )  # pour drude
        opt_prob.addVar(
            "gamma" + str(i),
            "c",
            lower=lb[3 + interm2 + 3 * i],
            upper=up[3 + interm2 + 3 * i],
            value=drudeinput[3 + interm2 + 3 * i],
        )  # pour drude
...
np.savetxt(fileoutputtime, outputtime, header="time \t E-field")
fileoutputtime.close()

outputfreq = abs(
    np.column_stack(
        (
            myglobalparameters.freq,
            myfitteddata.Spulse,
            np.real(myfitteddata.epsilon),
            np.imag(myfitteddata.epsilon),
            np.real(np.sqrt(myfitteddata.epsilon)),
            np.imag(np.sqrt(myfitteddata.epsilon)),
            np.real(monepsilon),
            np.imag(monepsilon),
            np.real(np.sqrt(monepsilon)),
            np.imag(np.sqrt(monepsilon)),
        )
    )
)
print(
...
np.savetxt(
    fileoutputfreq,
    outputfreq,
    header="Freq \t E-field \t real part of fitted epsilon \t imaginary part of fitted epsilon \t real part of fitted n \t imaginary part of fitted n \t real part of initial epsilon \t imaginary part of initial epsilon \t real part of initial n\t imaginary part of initial n",
)
