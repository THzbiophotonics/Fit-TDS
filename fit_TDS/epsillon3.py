...
def inverseProblem(freq, fctTrans, thick, FP, init, limitUp, phse):
    ...
    # measAngleTh = unwrap(angle(meas)) + phse * np.pi
    measAbsTh = abs(meas)
    ...
    for i in range(0, f.shape[0] - 1):
        ...
        # err_function_tf = lambda x: errorFct_TF(x[0], x[1], thick, f[i], fctTrans[i], FP)
        # res = minimize(err_function_tf, init[i], method="Nelder-Mead", tol=1e-6)
        # res = minimize(err_function_tf, init[i], method="BFGS", tol=1e-6)
    ...
    for i in range(0, f.shape[0] - 1):
        ...
        try:
            ...
            err_fct_hess = lambda x: errorFct_TF(x[0], x[1],  thick, f[i], fctTrans[i],FP,print_bool=False)
...
def dielcal(mytransferfunction, z, myglobalparameters,FP, phse,scattering=None):
    ...
    for compt in range(0, 3):  # the max of this loop was choosen to optimize the effective index for a 250micron thick quartz substrate it gives good result at 1/1000 but shows weird convergeance (larger is not better so this may had to be changed with the sample)
        ...
        phase = np.unwrap(myangleTF + myangleTrans) + phse * np.pi # we are taking the phase from the transfer function
        # phase = np.unwrap(myangleTF + myangleTrans)   # we are taking the phase from the transfer function
        # phase_diff = np.diff(phase)
        # phase_bool = np.where(phase_diff>180)
        # print(f"phase diff: {phase_bool}")
    ...
    ref_index[0] = 1
    # for i,k in enumerate(kaprox):
        # if k.real > 1e-2:
            # print(f"k guess is large")
            # #print(kaprox.real[i])
            # #print(mytransferfunction[i])
            # #print(myglobalparameters.w[i]/1e12)
    # print(ref_index)

    ref_index2 = inverseProblem(myglobalparameters.w / 2 / np.pi, mytransferfunction2, zz, FP, ref_index, 6e12, phse)
